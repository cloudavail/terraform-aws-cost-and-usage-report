...
resource "aws_s3_bucket" "cost_and_usage_report" {
  bucket = "cloudavail-cost-and-usage-report"
}
...
# the policy below is provided by AWS through the AWS Console when creating a aws_bcmdataexports_export type resource
# the policy does not follow least permission principle so a ticket has been filed:
# https://support.console.aws.amazon.com/support/home#/case/?displayId=173732174900900&language=en
  ...
  statement {
    ...
    condition {
      ...
      values = [
        ...
        "arn:aws:bcm-data-exports:us-east-1:${data.aws_region.current.name}:export/*"
      ]
    }
    resources = [
      aws_s3_bucket.cost_and_usage_report.bucket,
      "${aws_s3_bucket.cost_and_usage_report.bucket}/*",
    ]
  }
}

resource "aws_bcmdataexports_export" "standard_data_export" {
  export {
    name = "standard-data-export"
    data_query {
    ...
    destination_configurations {
      s3_destination {
        ...
        s3_prefix = ""
        s3_region = aws_s3_bucket.cost_and_usage_report.region
